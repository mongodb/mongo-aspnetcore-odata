...
if [[ $OS =~ [Ww]indows.* ]]; then
  ...
  powershell.exe "$DOTNET_SDK_PATH"/dotnet-install.ps1 -Channel 5.0 -InstallDir "$DOTNET_SDK_PATH" -NoPath -Runtime aspnetcore
  ...
  echo "Installing ASP.NET Core 5.0 runtime..."
  bash "$DOTNET_SDK_PATH"/dotnet-install.sh --channel 5.0 --install-dir "$DOTNET_SDK_PATH" --no-path --runtime aspnetcore
  echo "Installing .NET LTS SDK..."
