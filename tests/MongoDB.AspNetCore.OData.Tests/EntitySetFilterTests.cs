...
public class EntitySetFilterTests
{
    ...
    private static readonly (string Name, string Url, Func<City, bool> ItemValidator)[] s_filterTestCases =
    {
        ...
        (
            "substring_start_deep",
            "/odata/Cities?$filter=toupper(tolower(substring(Name, 0))) eq 'NEW YORK'",
            item => item.Name.Substring(0).ToLower().ToUpper() == "NEW YORK"
        ),
        (
            "substring_start_indirect",
            "/odata/Cities?$filter=tolower(substring(Name, 0)) eq 'new york'",
            item => item.Name.Substring(0).ToLower() == "new york"
        ),
        (
            "substring_start_nested",
            "/odata/Cities?$filter=substring(Name, length(substring(Name, 0))) eq ''",
            item => item.Name.Substring(item.Name.Substring(0).Length) == ""
        ),
        (
            "substring_start_reversed",
            "/odata/Cities?$filter= 'New York' eq substring(Name, 0)",
            item => "New York" == item.Name.Substring(0)
        ),
        (
        ...
        (
            "substring_start_length_deep",
            "/odata/Cities?$filter=tolower(trim(substring(Name, 0, 4))) eq 'new'",
            item => item.Name.Substring(0, 4).ToLower().Trim() == "new"
        ),
        (
            "substring_start_length_indirect",
            "/odata/Cities?$filter=tolower(substring(Name, 0, 4)) eq 'new '",
            item => item.Name.Substring(0, 4).ToLower() == "new "
        ),
        (
            "substring_start_length_nested",
            "/odata/Cities?$filter=substring(Name, length(substring(Name, 0)) sub length(substring(Name, 0)), length(substring(Name, 0))) eq 'New York'",
            item => item.Name.Substring(item.Name.Substring(0).Length - item.Name.Substring(0).Length,
                item.Name.Substring(0).Length) == "New York"
        ),
        (
            "substring_start_length_reversed",
            "/odata/Cities?$filter='York' eq substring(Name, 4, 4)",
            item => "York" == item.Name.Substring(4, 4)
        ),
        (
            "concat",
        ...
        (
            "fractionalseconds",
            "/odata/Cities?$filter=fractionalseconds(Date) eq 0",
            item => item.Date.TimeOfDay.Milliseconds == 0
        ),
        (
            "time",
